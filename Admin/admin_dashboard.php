...
.chartAreaWrapper {
  ...
  <body>
      ...
      <div class="offcanvas-body p-0">
        <nav class="navbar-light">
          <ul class="navbar-nav">
            ...
            <li>
              <a href="admin_dashboard.php" class="nav-link px-3 text-primary">
                <span class="me-2">
    ...
    <main class="mt-5 pt-3">
      <div class="container-fluid">
        <div class="row">
          <p class="col-md-12 fw-bold fs-4">Hello, Admin!</p>
        </div>

        <div class="row g-3 my-2">
          <div class="col-lg-3">
            <div
              class="p-3 blue-bg text-white shadow-sm d-flex justify-content-around align-items-center rounded"
            >
              <div>
                <h3 class="fs-3"><?php 
                $ida="SELECT * FROM ideas";
                $resida = mysqli_query($Connect, $ida);
                if (mysqli_num_rows($resida) > 0) {
                  echo mysqli_num_rows($resida);
                }
                else
                {
                  echo "0";
                 ...
                 ?></h3>
                <p class="fs-6">Total Ideas</p>
              </div>
              <i
                class="fa-solid fa-lightbulb fs-2 secondary-color border rounded-full light-bg ms-2 p-3"
              ></i>
            </div>
          </div>
          <div class="col-lg-3">
            <div
              class="p-3 orange-color text-white shadow-sm d-flex justify-content-around align-items-center rounded"
            >
              <div>
                <h3 class="fs-3"><?php 
                $dp="SELECT * FROM department";
                $resdp = mysqli_query($Connect, $dp);
                if (mysqli_num_rows($resdp) > 0) {
                  echo mysqli_num_rows($resdp);
                }
                else
                {
                  echo "0";
                 ...
                 ?></h3>
                <p class="fs-6">Departments</p>
              </div>
              <i
                class="fa-regular fa-building fs-2 secondary-color border rounded-full light-bg ms-2 p-3"
              ></i>
            </div>
          </div>
          <div class="col-lg-3">
            <div
              class="p-3 grey-bg text-white shadow-sm d-flex justify-content-around align-items-center rounded"
            >
              <div>
                <h3 class="fs-3">
                  <?php 
                $stff="SELECT * FROM users where userRoleID=3";
                $resstff = mysqli_query($Connect, $stff);
                if (mysqli_num_rows($resstff) > 0) {
                  echo mysqli_num_rows($resstff);
                }
                else
                {
                  echo "0";
                ...
                <p class="fs-6">Total Staff</p>
              </div>
              <i
                class="fa-solid fa-user fs-2 secondary-color border rounded-full light-bg ms-2 p-3"
              ></i>
            </div>
          </div>
          <div class="col-lg-3">
            <div
              class="p-3 black-bg text-white shadow-sm d-flex justify-content-around align-items-center rounded"
            >
              <div>
                <h3 class="fs-3"><?php 

                $ida1="SELECT * FROM ideas";
                $totcmt=0;
                $resida1 = mysqli_query($Connect, $ida1);
                if (mysqli_num_rows($resida1) > 0) {
                  while($row=mysqli_fetch_array($resida1))
                  {
                    $totcmt+=$row['commentCount'];
                  }
                  echo $totcmt;
                }
                else
                {
                 ...
                 ?></h3>
                <p class="fs-6">Comments</p>
              </div>

              <i
                class="fa-regular fa-message fs-2 secondary-color border rounded-full light-bg ms-2 p-3"
              ></i>
            </div>
          </div>
        </div>
        ...
        <div class="row">
          <div class="col-md-6">
            <div class="card">
              <div class="card-header">Idea Per Category</div>
              <div class="card-body">
...
<div class="chartWrapper"> 
         ...
         <canvas id="axis1" height="300" width="0"></canvas> 
          ...
          <div class="col-md-6">
            <div class="card">
              <div class="card-header">Idea Per Department</div>
              <div class="card-body">
                <div class="chartWrapper"> 
         <div class="chartAreaWrapper"> 
              ...
              <div class="card-body">
                <div class="table-responsive">
                  ...
                  >
                    ...
                    <tbody>
                      <?php   $dep="SELECT * FROM department order by departmentID";
      ...
      $resusers = mysqli_query($Connect, $user);
        if (mysqli_num_rows($resusers) > 0) {
          $staffcount=mysqli_num_rows($resusers) ;
    while($arr2 = mysqli_fetch_array($resusers)){
      ...
      $no =mysqli_num_rows($residea) ;
        $noid+=$no;
 
          while($arr3 = mysqli_fetch_array($residea)){
          $totalcomments+=$arr3['commentCount'];
      $totalreactions+=$arr3['likeCount']+$arr3['dislikeCount'];
      $totalviews+=$arr3['viewCount'];
    }
    }
    else
    {
...
?>
                      <tr>
                        <td><?php echo  $arr1['departmentName']; ?></td>
                        <td><?php echo  $staffcount; ?></td>
                        <td><?php echo  $noid; ?></td>
                        <td><?php echo  $totalcomments; ?></td>
                        <td><?php echo  $totalreactions; ?></td>
